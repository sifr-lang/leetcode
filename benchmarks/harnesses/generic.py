...
from harnesses.sifr_prelude import SIFR_PRELUDE
...
def single_sifr_runner_body(function: str, runner: dict[str, Any], owned_args: set[str]) -> str:
    ...
    copied_args = set(runner["call"].get("copy_args", []))
    call = sifr_call(function, runner["call"]["args"], runner["input"]["bindings"], owned_args, copied_args)
    expected = runner["expected"]
    expected_type = expected["type"]
...
def sifr_call(
    ...
    for arg in args:
        ...
        elif binding and (arg in owned_args or arg in copied_args):
            rendered_args.append(sifr_copy_expr(arg, binding["type"]))
        else:
            rendered_args.append(arg)
    return f"{function}({', '.join(rendered_args)})"

def sifr_copy_expr(arg: str, binding_type: str) -> str:
    copy_functions = {
        "list[int]": "_bench_copy_list_int",
        "list[str]": "_bench_copy_list_str",
        "list[float]": "_bench_copy_list_float",
        "matrix[int]": "_bench_copy_matrix_int",
        "matrix[str]": "_bench_copy_matrix_str",
    }
    copy_function = copy_functions.get(binding_type)
    if copy_function is None:
        return arg
    return f"{copy_function}({arg})"
