...
function shortestSubarray(nums, k) {
  ...
  const pre = [0];
  for (const i of nums) pre.push(pre[pre.length - 1] + i);
  let ans = size + 1;
  const monoq = [];
  for (let i = 0; i < pre.length; i++) {
    const val = pre[i];
    while (monoq.length && val <= pre[monoq[monoq.length - 1]]) monoq.pop();
    while (monoq.length && val - pre[monoq[0]] >= k) ans = Math.min(ans, i - monoq.shift());
    monoq.push(i);
  }
...
if (require.main === module) {
  ...
  assert.strictEqual(shortestSubarray([1], 1), 1);
  assert.strictEqual(shortestSubarray([1, 2], 4), -1);
  assert.strictEqual(shortestSubarray([2, -1, 2], 3), 3);
}
