...
function canConstruct(ransomNote, magazine) {
  const rCounter = new Map();
  const mCounter = new Map();
  for (const c of ransomNote) rCounter.set(c, (rCounter.get(c) || 0) + 1);
  for (const c of magazine) mCounter.set(c, (mCounter.get(c) || 0) + 1);
  for (const c of ransomNote) if ((mCounter.get(c) || 0) < (rCounter.get(c) || 0)) return false;
  return true;
}
