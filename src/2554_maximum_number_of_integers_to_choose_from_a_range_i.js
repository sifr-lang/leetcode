...
function maxCount(banned, n, maxSum) {
  const nums = new Map();
  for (let x = 1; x <= n; x++) nums.set(x, 1);
  for (const i of banned) if (nums.get(i)) nums.delete(i);
  let total = 0, count = 0;
  for (const i of nums.keys()) {
    total += i;
    if (total <= maxSum) count += 1;
    else break;
  }
  return count;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(maxCount([1, 6, 5], 5, 6), 2);
  assert.strictEqual(maxCount([1, 2, 3, 4, 5, 6, 7], 8, 1), 0);
  assert.strictEqual(maxCount([11], 7, 50), 7);
}
