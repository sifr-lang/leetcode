...
function findJudge(n, trust) {
  ...
  for (const [src, dst] of trust) {
    delta.set(src, (delta.get(src) || 0) - 1);
    delta.set(dst, (delta.get(dst) || 0) + 1);
  }
  for (let i = 1; i <= n; i++) if ((delta.get(i) || 0) === n - 1) return i;
  return -1;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(findJudge(2, [[1, 2]]), 2);
  assert.strictEqual(findJudge(3, [[1, 3], [2, 3]]), 3);
  assert.strictEqual(findJudge(3, [[1, 3], [2, 3], [3, 1]]), -1);
}
