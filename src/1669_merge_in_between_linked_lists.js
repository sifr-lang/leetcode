...
function mergeInBetween(list1, a, b, list2) {
  let curr = list1;
  let i = 0;
  while (i < a - 1) { curr = curr.next; i += 1; }
  const head = curr;
  while (i <= b) { curr = curr.next; i += 1; }
  head.next = list2;
...
if (require.main === module) {
  ...
  assert.strictEqual(
    listNodeToString(mergeInBetween(buildListNode([10, 1, 13, 6, 9, 5]), 3, 4, buildListNode([1000000, 1000001, 1000002]))),
    listNodeToString(buildListNode([10, 1, 13, 1000000, 1000001, 1000002, 5])),
  );
  assert.strictEqual(
    listNodeToString(mergeInBetween(buildListNode([0, 1, 2, 3, 4, 5, 6]), 2, 5, buildListNode([1000000, 1000001, 1000002, 1000003, 1000004]))),
    listNodeToString(buildListNode([0, 1, 1000000, 1000001, 1000002, 1000003, 1000004, 6])),
  );
}
