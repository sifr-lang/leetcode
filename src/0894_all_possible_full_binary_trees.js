...
function allPossibleFBT(n) {
  const dp = new Map([[0, []], [1, [new TreeNode()]]]);
  function backtrack(count) {
    if (dp.has(count)) return dp.get(count);
    const res = [];
    for (let l = 0; l < count; l++) {
      const r = count - 1 - l;
      const leftTrees = backtrack(l), rightTrees = backtrack(r);
      for (const t1 of leftTrees) for (const t2 of rightTrees) res.push(new TreeNode(0, t1, t2));
    }
    dp.set(count, res);
    return res;
  }
...
function sortedTreeStrings(nodes) { return nodes.map(treeToString).sort(); }

module.exports = { allPossibleFBT };

if (require.main === module) {
  const assert = require('assert');
  assert.deepStrictEqual(sortedTreeStrings(allPossibleFBT(7)), [
  ...
  assert.deepStrictEqual(sortedTreeStrings(allPossibleFBT(3)), ['0(0(None,None),0(None,None))']);
}
