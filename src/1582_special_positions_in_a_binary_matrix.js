...
function numSpecial(mat) {
  ...
  const rowCount = Array(m).fill(0), colCount = Array(n).fill(0);
  let res = 0;
  for (let r = 0; r < m; r++) for (let c = 0; c < n; c++) if (mat[r][c] === 1) { rowCount[r] += 1; colCount[c] += 1; }
  for (let r = 0; r < m; r++) for (let c = 0; c < n; c++) if (mat[r][c] === 1 && rowCount[r] === 1 && colCount[c] === 1) res += 1;
  return res;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(numSpecial([[1, 0, 0], [0, 0, 1], [1, 0, 0]]), 1);
  assert.strictEqual(numSpecial([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), 3);
}
