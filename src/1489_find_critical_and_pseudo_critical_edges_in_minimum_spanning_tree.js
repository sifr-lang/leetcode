...
function findCriticalAndPseudoCriticalEdges(n, edges) {
  ...
  for (const [v1, v2, w] of edges) if (uf.union(v1, v2)) mstWeight += w;
  const critical = [], pseudo = [];
  for (const [n1, n2, eWeight, i] of edges) {
    let weight = 0;
    uf = new UnionFind(n);
    for (const [v1, v2, w, j] of edges) if (i !== j && uf.union(v1, v2)) weight += w;
    if (Math.max(...uf.rank) !== n || weight > mstWeight) { critical.push(i); continue; }
    uf = new UnionFind(n);
    uf.union(n1, n2);
    weight = eWeight;
    for (const [v1, v2, w] of edges) if (uf.union(v1, v2)) weight += w;
    if (weight === mstWeight) pseudo.push(i);
  }
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(findCriticalAndPseudoCriticalEdges(5, [[0, 1, 1], [1, 2, 1], [2, 3, 2], [0, 3, 2], [0, 4, 3], [3, 4, 3], [1, 4, 6]]), [[0, 1], [2, 3, 4, 5]]);
  assert.deepStrictEqual(findCriticalAndPseudoCriticalEdges(4, [[0, 1, 1], [1, 2, 1], [2, 3, 1], [0, 3, 1]]), [[], [0, 1, 2, 3]]);
}
