...
function numSubmatrixSumTarget(matrix, target) {
  ...
  for (let r1 = 0; r1 < ROWS; r1++) {
    for (let r2 = r1; r2 < ROWS; r2++) {
      ...
      for (let c = 0; c < COLS; c++) {
        const curSum = subSum[r2][c] - (r1 > 0 ? subSum[r1 - 1][c] : 0);
...
if (require.main === module) {
  ...
  assert.strictEqual(numSubmatrixSumTarget([[0, 1, 0], [1, 1, 1], [0, 1, 0]], 0), 4);
  assert.strictEqual(numSubmatrixSumTarget([[1, -1], [-1, 1]], 0), 5);
  assert.strictEqual(numSubmatrixSumTarget([[904]], 0), 0);
}
