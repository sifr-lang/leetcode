...
const BAD_VERSION = 4;
function isBadVersion(version) { return version >= BAD_VERSION; }
function firstBadVersion(n) {
  let l = 1, r = n;
  while (l < r) {
    const v = Math.floor((l + r) / 2);
...
if (require.main === module) {
  ...
  assert.strictEqual(firstBadVersion(5), 4);
  assert.strictEqual(firstBadVersion(4), 4);
}
