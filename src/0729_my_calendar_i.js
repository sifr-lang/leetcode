...
class CalendarNode {
  constructor(start, end) { this.start = start; this.end = end; this.left = null; this.right = null; }
}
class MyCalendar {
  constructor() { this.calendar = new CalendarNode(-1, -1); }
  book(start, end) {
    function bookHelper(cur, targetStart, targetEnd) {
      if (targetStart > cur.end) {
        if (!cur.right) { cur.right = new CalendarNode(targetStart, targetEnd); return true; }
        return bookHelper(cur.right, targetStart, targetEnd);
      } else if (targetEnd < cur.start) {
        if (!cur.left) { cur.left = new CalendarNode(targetStart, targetEnd); return true; }
        return bookHelper(cur.left, targetStart, targetEnd);
      }
      return false;
    }
    return bookHelper(this.calendar, start, end - 1);
  }
}
...
if (require.main === module) {
  ...
  assert.strictEqual(obj.book(10, 20), true);
  assert.strictEqual(obj.book(15, 25), false);
  assert.strictEqual(obj.book(20, 30), true);
}
