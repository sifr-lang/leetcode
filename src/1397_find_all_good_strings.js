...
function findGoodStrings(n, s1, s2, evil) {
  ...
  const arrE = Array.from(evil, (ch) => ch.charCodeAt(0));
  const lenE = evil.length;
  const next = Array(lenE).fill(0);
  ...
  function good(s) {
    ...
    const memo = new Map();
    function f(i, skip, reach, e) {
      if (e === lenE) return 0;
      if (i === lenA) return skip ? 0 : 1;
      const key = `${i},${skip ? 1 : 0},${reach ? 1 : 0},${e}`;
      if (memo.has(key)) return memo.get(key);
      const limit = reach ? arr[i] : z;
      let ans = 0;
      if (skip) ans += f(i + 1, true, false, 0);
      for (let c = a; c <= limit; c++) {
        let ee = e;
        while (ee > 0 && arrE[ee] !== c) ee = next[ee - 1];
        if (arrE[ee] === c) ee += 1;
        ans += f(i + 1, false, reach && c === limit, ee);
      }
      ans %= 1000000007;
      memo.set(key, ans);
      return ans;
  ...
  return (good(s2) - good(s1) + (s1.includes(evil) ? 0 : 1) + 1000000007) % 1000000007;
}

module.exports = { findGoodStrings };

if (require.main === module) {
  const assert = require('assert');
  assert.strictEqual(findGoodStrings(2, 'aa', 'da', 'b'), 51);
  assert.strictEqual(findGoodStrings(8, 'leetcode', 'leetgoes', 'leet'), 0);
  assert.strictEqual(findGoodStrings(2, 'gx', 'gz', 'x'), 2);
}
