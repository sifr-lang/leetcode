...
function canTraverseAllPairs(nums) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    while (f * f <= n) {
      if (n % f === 0) {
        if (factorIndex.has(f)) uf.union(i, factorIndex.get(f));
        else factorIndex.set(f, i);
        while (n % f === 0) n = Math.floor(n / f);
      }
      f += 1;
    }
    if (n > 1) {
      if (factorIndex.has(n)) uf.union(i, factorIndex.get(n));
      else factorIndex.set(n, i);
    }
  }
  return uf.count === 1;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(canTraverseAllPairs([2, 3, 6]), true);
  assert.strictEqual(canTraverseAllPairs([3, 9, 5]), false);
  assert.strictEqual(canTraverseAllPairs([4, 3, 12, 8]), true);
}
