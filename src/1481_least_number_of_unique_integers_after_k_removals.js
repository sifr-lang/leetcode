...
function findLeastNumOfUniqueInts(arr, k) {
  ...
  const freqList = Array(arr.length + 1).fill(0);
  for (const f of freq.values()) freqList[f] += 1;
  let res = freq.size;
  for (let f = 1; f < freqList.length; f++) {
    let remove = freqList[f];
    if (k >= f * remove) { k -= f * remove; res -= remove; }
    else { remove = Math.floor(k / f); res -= remove; break; }
  }
  return res;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(findLeastNumOfUniqueInts([5, 5, 4], 1), 1);
  assert.strictEqual(findLeastNumOfUniqueInts([4, 3, 1, 1, 3, 3, 2], 3), 2);
}
