...
function mostBooked(n, meetings) {
  ...
  const used = new MinHeap((a, b) => a[0] < b[0] || (a[0] === b[0] && a[1] < b[1]));
  const count = Array(n).fill(0);
  for (let [start, end] of meetings) {
    while (used.length && start >= used.peek()[0]) {
      const [, room] = used.pop();
      available.push(room);
    }
    if (!available.length) {
      const [endTime, room] = used.pop();
...
if (require.main === module) {
  ...
  assert.strictEqual(mostBooked(2, [[0, 10], [1, 5], [2, 7], [3, 4]]), 0);
  assert.strictEqual(mostBooked(3, [[1, 20], [2, 10], [3, 5], [4, 9], [6, 8]]), 1);
}
