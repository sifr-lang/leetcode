...
function checkIfPrerequisite(numCourses, prerequisites, queries) {
  ...
  function dfs(crs) {
    if (!prereqMap.has(crs)) {
      const s = new Set();
      prereqMap.set(crs, s);
      for (const pre of adj.get(crs) || []) for (const value of dfs(pre)) s.add(value);
    }
    prereqMap.get(crs).add(crs);
    return prereqMap.get(crs);
  }
  for (let crs = 0; crs < numCourses; crs++) dfs(crs);
  const res = [];
  for (const [u, v] of queries) res.push(prereqMap.get(v).has(u));
  return res;
}
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(checkIfPrerequisite(2, [[1, 0]], [[0, 1], [1, 0]]), [false, true]);
  assert.deepStrictEqual(checkIfPrerequisite(2, [], [[1, 0], [0, 1]]), [false, false]);
  assert.deepStrictEqual(checkIfPrerequisite(3, [[1, 2], [1, 0], [2, 0]], [[1, 0], [1, 2]]), [true, true]);
}
