...
function onesMinusZeros(grid) {
  ...
  const rowCount = Array.from({ length: m }, () => [0, 0]);
  const colCount = Array.from({ length: n }, () => [0, 0]);
  const res = [];
  for (let r = 0; r < m; r++) for (let c = 0; c < n; c++) {
    if (grid[r][c] === 1) { rowCount[r][1] += 1; colCount[c][1] += 1; }
    else { rowCount[r][0] += 1; colCount[c][0] += 1; }
  }
  for (let r = 0; r < m; r++) {
    const row = [];
    for (let c = 0; c < n; c++) row.push(rowCount[r][1] + colCount[c][1] - rowCount[r][0] - colCount[c][0]);
    res.push(row);
  }
  return res;
}
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(onesMinusZeros([[0, 1, 1], [1, 0, 1], [0, 0, 1]]), [[0, 0, 4], [0, 0, 4], [-2, -2, 2]]);
  assert.deepStrictEqual(onesMinusZeros([[1, 1, 1], [1, 1, 1]]), [[5, 5, 5], [5, 5, 5]]);
}
