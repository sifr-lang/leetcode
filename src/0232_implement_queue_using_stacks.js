...
class MyQueue {
  constructor() { this.append_stack = []; this.inverted_stack = []; }
  push(x) { this.append_stack.push(x); }
  pop() {
    if (!this.inverted_stack.length) while (this.append_stack.length) this.inverted_stack.push(this.append_stack.pop());
    return this.inverted_stack.pop();
  }
  peek() {
    if (!this.inverted_stack.length) while (this.append_stack.length) this.inverted_stack.push(this.append_stack.pop());
    return this.inverted_stack[this.inverted_stack.length - 1];
  }
  empty() { return !(this.append_stack.length || this.inverted_stack.length); }
}
...
if (require.main === module) {
  ...
  assert.strictEqual(obj.pop(), 1);
  assert.strictEqual(obj.empty(), false);
}
