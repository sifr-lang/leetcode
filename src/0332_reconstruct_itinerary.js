...
function findItinerary(tickets) {
  const adj = new Map();
  const res = [];
  for (const [src] of tickets) adj.set(src, []);
  for (const [src, dst] of tickets) adj.get(src).push(dst);
  for (const key of adj.keys()) adj.get(key).sort();
  function dfs(src) {
    if (adj.has(src)) {
      let destinations = adj.get(src).slice();
      while (destinations.length) {
        const dest = destinations[0];
        adj.get(src).shift();
        dfs(dest);
        destinations = adj.get(src).slice();
      }
    }
    res.push(src);
  }
  dfs('JFK');
  res.reverse();
  if (res.length !== tickets.length + 1) return [];
  return res;
}
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(findItinerary([['MUC', 'LHR'], ['JFK', 'MUC'], ['SFO', 'SJC'], ['LHR', 'SFO']]), ['JFK', 'MUC', 'LHR', 'SFO', 'SJC']);
  assert.deepStrictEqual(findItinerary([['JFK', 'SFO'], ['JFK', 'ATL'], ['SFO', 'ATL'], ['ATL', 'JFK'], ['ATL', 'SFO']]), ['JFK', 'ATL', 'JFK', 'SFO', 'ATL', 'SFO']);
}
