...
function finalPrices(prices) {
  ...
  for (let i = prices.length - 1; i >= 0; i--) {
    if (stack.length === 0) res.push(prices[i]);
    else if (stack.length && stack[stack.length - 1] <= prices[i]) res.push(prices[i] - stack[stack.length - 1]);
    else if (stack.length && stack[stack.length - 1] > prices[i]) {
      while (stack.length && stack[stack.length - 1] > prices[i]) stack.pop();
      if (stack.length === 0) res.push(prices[i]);
      else res.push(prices[i] - stack[stack.length - 1]);
    }
    stack.push(prices[i]);
  }
  res.reverse();
