...
function largestPerimeter(nums) {
  ...
  nums.sort((a, b) => b - a);
  while (nums.length && curSum <= nums[0] * 2) curSum -= nums.shift();
  return nums.length > 2 ? curSum : -1;
}
...
if (require.main === module) {
  ...
  assert.strictEqual(largestPerimeter([5, 5, 5]), 15);
  assert.strictEqual(largestPerimeter([1, 12, 1, 2, 5, 50, 3]), 12);
}
