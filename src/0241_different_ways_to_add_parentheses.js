...
function diffWaysToCompute(s) {
  function f(expr) {
    const res = [];
    for (let i = 0; i < expr.length; i++) {
      const c = expr[i];
      if ('+-*'.includes(c)) {
        for (const l of f(expr.slice(0, i))) {
          for (const r of f(expr.slice(i + 1))) {
            if (c === '+') res.push(l + r);
    ...
    return res.length ? res : [Number(expr)];
  }
  return f(s);
}
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(diffWaysToCompute('2-1-1'), [2, 0]);
}
