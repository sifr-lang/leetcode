...
function maximumOddBinaryNumber(s) {
  s = Array.from(s);
  let left = 0;
  for (let i = 0; i < s.length; i++) if (s[i] === '1') { [s[i], s[left]] = [s[left], s[i]]; left += 1; }
  [s[left - 1], s[s.length - 1]] = [s[s.length - 1], s[left - 1]];
  return s.join('');
}
...
if (require.main === module) {
  ...
  assert.strictEqual(maximumOddBinaryNumber('010'), '001');
  assert.strictEqual(maximumOddBinaryNumber('0101'), '1001');
}
