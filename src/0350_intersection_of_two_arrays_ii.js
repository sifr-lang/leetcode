...
function intersect(nums1, nums2) {
  ...
  for (const n of nums1) counter1.set(n, (counter1.get(n) || 0) + 1);
  for (const n of nums2) counter2.set(n, (counter2.get(n) || 0) + 1);
  const intersection = [];
  for (const [num, freq] of counter1.entries()) {
    const minFreq = Math.min(freq, counter2.get(num) || 0);
    for (let i = 0; i < minFreq; i++) intersection.push(num);
  }
  return intersection;
}
...
if (require.main === module) {
  ...
  assert.deepStrictEqual(intersect([4, 9, 5], [9, 4, 9, 8, 4]), [4, 9]);
}
